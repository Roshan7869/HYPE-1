...
export default function LiveAuctionsPage({ searchParams }: PageProps) {
  ...
  // Pick the right dataset based on the status filter
  const items =
    status === "upcoming"
      ? getUpcomingAuctions()
      : status === "sold"
  ...
  // Featured logic: first live item for hero, or first item if no live
  const featured =
    status === "live"
  ...
  const titleMap: Record<StatusFilter, { title: string; sub: string; count: string }> = {
    live: {
      title: "Bid on the drops",
      sub: "everybody wants.",
      count: "Live Now",
    },
    upcoming: {
      title: "Dropping soon.",
      sub: "Set your reminders.",
      count: "Upcoming",
    },
    sold: {
      title: "Recently sold.",
    ...
    <div className="page">
      ...
      <main className="flex-1 bg-sand">
        ...
        <section className="bg-cream-2 py-14">
          <div className="wrap">
            <div className="flex items-center gap-3 text-[12px] font-bold uppercase tracking-[0.18em] text-muted">
              {status === "live" ? (
                <>
                  <LiveBadge size="sm" />
              ...
              {(["live", "upcoming", "sold"] as const).map((s) => {
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      "flex h-10 items-center gap-2 rounded-full px-5 text-[13px] font-semibold transition-colors",
                      isActive
                        ? "bg-ink text-white"
